...
const getCountryInfo = function(code: string) {
    let alpha2 = null;
    let alpha3 = null;
    const altNames = [];
    ...
    if (info) {
        ...
        altNames.push(info.name);
        altNames.push(info.nativeName);
...
for (const letter1 of alphabet) {
    for (const letter2 of alphabet) {
        ...
        if (alpha2) {
            ...
            names.add(alpha2);
            names.add(alpha3);
            altNames.forEach((name) => names.add(name));
